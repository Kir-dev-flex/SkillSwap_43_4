import React from 'react';
import styles from './LikeIcon.module.css';

/**
 * Интерфейс пропсов компонента LikeIcon
 */
interface LikeIconProps {
  isLiked: boolean;
  onClick: (e?: React.MouseEvent) => void;
}

/**
 * Компонент для отображения интерактивной иконки "Лайк".
 * @param param0 {LikeIconProps} props - Пропсы компонента
 * @returns {JSX.Element} React-элемент кнопки с иконкой лайка
 */
export const LikeIcon: React.FC<LikeIconProps> = ({ isLiked, onClick }) => {
  const ariaLabel = isLiked ? 'убрать лайк' : 'поставить лайк';

  return (
    <button
      className={styles.likeButton}
      onClick={(e) => {
        e.stopPropagation();
        onClick(e);
      }}
      aria-label={ariaLabel}
      type='button'
    >
      {isLiked ? (
        // SVG для поставленного лайка
        <svg
          width='24'
          height='24'
          viewBox='0 0 24 24'
          fill='none'
          xmlns='http://www.w3.org/2000/svg'
          className={styles.likedIcon}
        >
          <path
            d='M7.5 3C4.46244 3 2 5.46245 2 8.5C2 14 8.5 19 12 20.1631C15.5 19 22 14 22 8.5C22 5.46245 19.5375 3 16.5 3C14.6398 3 12.9953 3.92345 12 5.3369C11.0047 3.92345 9.36015 3 7.5 3Z'
            fill='#ABD27A'
            stroke='#ABD27A'
            strokeWidth='2'
            strokeLinecap='round'
            strokeLinejoin='round'
          />
        </svg>
      ) : (
        // SVG для не поставленного лайка
        <svg
          width='24'
          height='24'
          viewBox='0 0 24 24'
          fill='none'
          xmlns='http://www.w3.org/2000/svg'
          className={styles.unlikedIcon}
        >
          <g clipPath='url(#clip0_53438_5821)'>
            <path
              d='M12 20.9535C11.7116 20.9535 11.4326 20.9163 11.2 20.8326C7.64651 19.614 2 15.2884 2 8.89767C2 5.64186 4.63256 3 7.86977 3C9.44186 3 10.9116 3.61395 12 4.71163C13.0884 3.61395 14.5581 3 16.1302 3C19.3674 3 22 5.65116 22 8.89767C22 15.2977 16.3535 19.614 12.8 20.8326C12.5674 20.9163 12.2884 20.9535 12 20.9535ZM7.86977 4.39535C5.40465 4.39535 3.39535 6.41395 3.39535 8.89767C3.39535 15.2512 9.50698 18.786 11.6558 19.5209C11.8233 19.5767 12.186 19.5767 12.3535 19.5209C14.493 18.786 20.614 15.2605 20.614 8.89767C20.614 6.41395 18.6047 4.39535 16.1395 4.39535C14.7256 4.39535 13.414 5.05581 12.5674 6.2C12.307 6.55349 11.7116 6.55349 11.4512 6.2C10.586 5.04651 9.28372 4.39535 7.86977 4.39535Z'
              fill='#253017'
            />
          </g>
          <defs>
            <clipPath id='clip0_53438_5821'>
              <rect width='24' height='24' fill='white' />
            </clipPath>
          </defs>
        </svg>
      )}
    </button>
  );
};
