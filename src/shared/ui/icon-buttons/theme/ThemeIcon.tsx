...
/**
 * Интерфейс пропсов компонента иконки смены темы
 */
...
/**
 * Компонент иконки для переключения между светлой и тёмной темами
 * @param props - Пропсы компонента, содержащие состояние темы и обработчик клика
 * @returns {JSX.Element} Кнопку с иконкой смены темы
 */
const ThemeIcon: React.FC<ThemeIconProps> = ({ isDark, onClick }) => {
  const ariaLabel = isDark ? 'Включить светлую тему' : 'Включить тёмную тему';
  const fillColor = isDark ? '#EBE5C5' : '#253017';
